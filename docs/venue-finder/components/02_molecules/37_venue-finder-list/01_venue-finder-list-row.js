...
(function (window, $) {
  ...
  $.widget(ns + name, {
    ...
    _initElements: (that, opts) => {
      ...
      //render the wrap
      that.wrap = $(`<a class="${CSSCLS.WRAP}" href='${opts.data.DetailUrl.substring(0, opts.data.DetailUrl.lastIndexOf('/'))}'></a>`);
      //render the list
    ...
    _loadDetails: (that, opts) => {
      ...
      GTC.Utils.request.postJSON({
        ...
        url: GTC.Utils.urls.blockDetails() + opts.data.DetailUrl.substring(opts.data.DetailUrl.lastIndexOf('/'), opts.data.DetailUrl.length),
        onDone: (data) => {
