...
(function (window, $) {
  ...
  $.widget(ns + name, {
    ...
    _createHeader: (that, opts) => {
      ...
      that.hdrLink = $(`<a class="${CSSCLS.ARTICLE.HEADER.WRAP} ${CSSCLS.ARTICLE.HEADER.LINK}" title="${opts.desc}" target="_blank" href="${opts.link.substring(0, opts.link.lastIndexOf('/'))}"><span class="${CSSCLS.ARTICLE.HEADER.TYPE}">${opts.type}</span> ${that._createRatingHtml(that, opts)} <h4 class="${CSSCLS.ARTICLE.HEADER.NAME}">${opts.title}</h4></a>`);
